...
| 模式 | 处理流程 | 适用场景 |
| --- | --- | --- |
| 智能模式（默认） | 优先中英文字幕；不可用时下载并分段分析音频，再汇总 | 日常观看，尽量减少上传和 API 用量 |
| 直接听音频 | 下载 / 提取音频 → 分段笔记 → 汇总 | 不需要逐字稿，字幕质量较差 |
| 完整转写 | 下载 / 提取音频 → 分段完整转写 → 文字总结 | 需要保留原文、核对内容 |

本地字幕文件在所有模式下直接按文字处理。直接音频总结不会伪装成逐字稿。转写由 Gemini 在云端完成，当前未集成本地 Whisper。音频理解不读取视频画面，无法还原未被口述的图表或字幕信息。
...
B 站风控 / 412、登录权限、地区与网络状态会影响提取，无法保证任意链接成功。下载失败时先确认视频在浏览器能播放，设置有效 Cookie，稍后重试或更新：
...
bili_notes/pipeline.py     字幕优先、转写、分段与分层总结
...
测试使用假 API 响应，覆盖取消、重试、缓存、原始文件保护、字幕优先与回退、长文本分块、云端文件清理、真实 FFmpeg 转码和 Qt 界面交互。测试不会调用付费模型，也不会读取个人 Cookie 或真实密钥。测试通过不等于已完成真实 B 站 + Gemini 端到端验证。
